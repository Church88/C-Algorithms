...
foreach x ( `seq 0 20` )
	if (-e $dir/inputfile$x) then
		echo "Test $x : $a < $dir/inputfile$x | diff -w - $dir/outputfile$x"
		$a < $dir/inputfile$x | diff -w - $dir/outputfile$x
		if (`$a < $dir/inputfile$x | diff -w - $dir/outputfile$x | wc -l` == 0) then
			echo "TEST PASSED!";
		endif
	endif
end
